  ...
  return dbQueue.execute(async () => {
    try {
      ...
      if (error instanceof SyntaxError) {
        ...
        try {
          const badData = await fs.readFile(DB_FILE, 'utf-8');
          await fs.writeFile(`${DB_FILE}.corrupted.${Date.now()}`, badData, 'utf-8');
        } catch (e) {}
        return JSON.parse(JSON.stringify(INITIAL_DB));
      }
      throw error;
    ...
    try {
      ...
      if (error.code === 'ENOENT' || error instanceof SyntaxError) {
        if (error instanceof SyntaxError) {
          ...
          try {
             await fs.writeFile(`${DB_FILE}.bak.${Date.now()}`, await fs.readFile(DB_FILE, 'utf-8'), 'utf-8');
          } catch(e){}
        }
